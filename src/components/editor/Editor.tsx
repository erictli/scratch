...
import { handleTaskListClipboardCopy } from "./taskListClipboard";
import {
  pasteTableTsv,
...
import { getEditorSaveStatus } from "../../lib/editorSaveStatus";
...
export function Editor({
  ...
  const titleBarNoteInfo = currentNote
    ...
    : null;
  const saveStatus = getEditorSaveStatus({
    hasSaveConflict,
    hasExternalChanges,
    isSaving,
    isDirty,
  ...
  useEffect(() => {
    if (!previewMode) return;
    return previewMode.registerPersistenceController({
      flush: flushAllPendingSaves,
      getDraft: getOpenDraftSnapshot,
    });
  }, [flushAllPendingSaves, getOpenDraftSnapshot, previewMode]);
  ...
  const editor = useEditor({
    ...
    editorProps: {
      ...
      handleDOMEvents: {
        ...
        copy: handleTaskListClipboardCopy,
      },
      ...
      handlePaste: (_view, event) => {
        ...
        if (isInsideTableCell && currentEditor && pasteTableTsv(currentEditor, text)) {
          return true;
        }
  ...
  useEffect(() => {
      ...
      if (
        ...
        if (needsSaveRef.current) {
          void flushPendingSave().catch((error) => {
            console.error("Failed to flush renamed note:", error);
          });
        }
        return;
      }
    ...
    if (!isSameNote && needsSaveRef.current) {
      void flushPendingSave().catch((error) => {
        console.error("Failed to flush previous note:", error);
      });
    }
  ...
  // Save barriers run before note/workspace/settings/window transitions. React
  // cleanup cannot await I/O, so it must never fire-and-forget the only draft.
  useEffect(() => {
    return () => {
      if (saveTimeoutRef.current) {
        clearTimeout(saveTimeoutRef.current);
