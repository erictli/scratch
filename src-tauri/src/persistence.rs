...
pub fn content_revision(content: &str) -> ContentRevision {
    let digest = crate::sha256::digest(content.as_bytes());
...
/// The per-path lock is process-local; external editors can still race, so the
/// revision check and atomic filesystem operation remain authoritative.
