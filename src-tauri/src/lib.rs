...
fn restorable_window_sessions(
    ...
    let mut records: Vec<_> = config
        ...
        .iter()
        .filter(|(label, record)| {
            !label.starts_with("preview-") && workspace_is_available(&record.workspace)
        })
...
fn build_application_menu(
    ...
    let application_menu_name = app_handle.package_info().name.clone();
...
fn suppress_full_windows_for_preview(app: &AppHandle) {
    let Some(state) = app.try_state::<AppState>() else {
        return;
    };
...
fn file_manager_reveal_command(
    platform: &str,
    path: &Path,
) -> Result<(String, Vec<std::ffi::OsString>), String> {
    match platform {
        "macos" => Ok((
            "open".to_string(),
...
fn validated_note_path_for_reveal(notes_root: &Path, id: &str) -> Result<PathBuf, String> {
...
#[tauri::command]
async fn reveal_note_in_file_manager(
    id: String,
    window: WebviewWindow,
    state: State<'_, AppState>,
) -> Result<(), String> {
    ...
    let file_path = validated_note_path_for_reveal(Path::new(&folder), &id)?;

    let (program, args) = file_manager_reveal_command(std::env::consts::OS, &file_path)?;
    let status = tauri::async_runtime::spawn_blocking(move || {
        std::process::Command::new(program).args(args).status()
    })
    .await
    .map_err(|error| format!("File manager task failed: {}", error))?
    .map_err(|error| format!("Failed to reveal note: {}", error))?;
...
pub fn run() {
    ...
    let app = tauri::Builder::default()
        ...
        .setup(|app| {
            ...
            if opened_preview {
                suppress_full_windows_for_preview(app.handle());
            }
    ...
    app.run(|_app_handle, _event| {
        ...
        if let tauri::RunEvent::Opened { urls } = _event {
            ...
            if let Some(paths) = defer_opened_markdown_paths_if_needed(_app_handle, paths) {
                ...
                if opened_preview {
                    suppress_full_windows_for_preview(_app_handle);
                }
...
mod file_manager_tests {
    use super::{file_manager_reveal_command, validated_note_path_for_reveal};
    use std::ffi::OsString;
    use std::path::Path;

    #[test]
    fn macos_reveals_the_file_in_finder() {
        let (program, args) =
            file_manager_reveal_command("macos", Path::new("/notes/Project plan.md")).unwrap();
        ...
        assert_eq!(
            ...
            vec![
                OsString::from("-R"),
                OsString::from("/notes/Project plan.md")
            ]
        );
    }

    #[test]
    fn windows_selects_the_file_in_explorer() {
        let (program, args) =
            file_manager_reveal_command("windows", Path::new("C:\\Notes\\Project plan.md"))
                .unwrap();
        ...
        assert_eq!(
            ...
            vec![
                ...
                OsString::from("C:\\Notes\\Project plan.md")
            ]
        );
    }
    ...
    fn linux_opens_the_containing_directory() {
        let (program, args) =
            file_manager_reveal_command("linux", Path::new("/notes/Project plan.md")).unwrap();

        assert_eq!(program, "xdg-open");
        assert_eq!(args, vec![OsString::from("/notes")]);
    }

    #[test]
    fn rejects_unsupported_platforms() {
        let result = file_manager_reveal_command("unknown", Path::new("/notes/Project plan.md"));

        assert_eq!(result.unwrap_err(), "Unsupported platform");
    }

    #[test]
    fn rejects_note_ids_that_escape_the_notes_folder() {
        let result = validated_note_path_for_reveal(Path::new("/notes"), "../Secrets");

        assert_eq!(
            result.unwrap_err(),
            "Invalid note ID: parent directory references not allowed"
        );
    }

    #[test]
    fn rejects_missing_note_files() {
        let result = validated_note_path_for_reveal(Path::new("/notes"), "Missing");

        assert_eq!(result.unwrap_err(), "Note file not found");
    }
}
